...
RAYA PROFILE STRUCTURE — evaluate the profile as Raya actually presents it:
- One main profile photo is the decisive first impression.
- The user may add an unlimited number of additional photos that viewers click through. Photos have no captions, prompts, or writing attached, so every image must communicate its signal visually and stand on its own. Do not impose an artificial ideal photo count; judge whether every included photo earns its place, whether the sequence builds intrigue, and where repetition weakens the edit.
- The profile displays age, job title, Instagram handle, and location. Evaluate whether these identity signals feel credible, polished, and coherent with the visual story. Never claim to have visited or analyzed the linked Instagram account.
- The Bio is freeform. It should add private texture, voice, or an intriguing personal detail rather than repeat the résumé-like metadata.
- The My Cities bubble has separate "Lives in" and "From" fields. Treat this as a belonging and lifestyle signal: where the person is rooted, where they move, and what social world they plausibly inhabit.
- The Saved Places bubble lists restaurants, bars, and clubs the user frequents. Evaluate the collection as a map of taste and real habits, not a prestige checklist. Reward a specific, coherent point of view; flag generic luxury name-dropping or a list that sends the wrong lifestyle signal.
- The Interests bubble contains a small number of broad interests such as "Having fun," "Family & friends," "Music," or "Side quests." Because individual labels are broad, judge the combination and whether the photos, bio, places, and song make those interests feel specific and credible.
- The Profile Song can be any song. Treat it as a high-signal editorial choice that conveys taste, mood, era, personality, and intended audience. Flag choices that feel generic, overly calculated, tonally inconsistent, or accidentally communicate the wrong energy.
- Raya recommendations must explicitly consider the main photo, click-through photo edit, Bio, identity metadata, My Cities, Saved Places, Interests, and Profile Song whenever those elements are visible or provided. The final advice should make these modules feel like one coherent, selective personal world.
